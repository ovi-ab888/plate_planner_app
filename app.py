...
class MCTSNode:
    ...
    def best_child(self, c_param=1.4):
        ...
        for child in self.children:
            if child.visits == 0: ucb = float('inf')
            else: ucb = (child.score / child.visits) + c_param * math.sqrt(2 * math.log(self.visits) / child.visits)
...
if input_mode == "Manual Input":
    ...
    n = st.number_input("🏷️ Number of Items", 1, 500, 1, key="manual_n_items")
    ...
    if uploaded_file is not None:
        try:
            ...
            if pdf_items:
                ...
                st.error("❌ পিডিএফ থেকে কোনো ডাটা রিড করা যায়নি। নিশ্চিত করুন এটি সঠিক WorkOrder ফরমেট কিনা।")
        except Exception as e:
            st.error(f"❌ ফাইল প্রসেস করতে ত্রুটি হয়েছে: {str(e)}")
    st.markdown('</div>', unsafe_allow_html=True)
...
if generate_clicked:
    ...
    with st.spinner("🔄 Running 10 algorithms simultaneously..."):
        ...
        results["V8 - Integer Solver"] = v8_optimizer(demand, cap, maxp) if v8_optimizer(demand, cap, maxp) else v5_optimizer(demand, cap, maxp)
