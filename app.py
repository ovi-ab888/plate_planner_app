...
def extract_qty_from_pdf(pdf_file):
    ...
    try:
        ...
        # Look for patterns like product codes with quantities
        lines = all_text.split('\n')
        
        # Pattern for: "95 MMW26 2173783000012 1" -> where last number is quantity
        pattern1 = r'([A-Z0-9\s]{10,})\s+(\d{1,5})\s*$'
        matches = re.findall(pattern1, all_text, re.MULTILINE)
        for match in matches:
            tag = match[0].strip()
            qty = int(match[1])
            if tag and 1 <= qty <= 1000000 and len(tag) > 3:
                if tag not in quantities:
                    quantities[tag] = qty
        
        # Pattern for lines with numbers at the end
        for line in lines:
            line = line.strip()
            if not line or len(line) < 5:
                continue
            
            # Skip lines with common non-product keywords
            skip_keywords = ['Total', 'Value', 'USD', 'Special', 'Instruction', 'Date', 'Page', 'Order', 'Work', 'Information']
            if any(keyword in line for keyword in skip_keywords):
                continue
            
            # Check if line ends with a reasonable number
            parts = line.split()
            if len(parts) >= 2:
                last_part = parts[-1]
